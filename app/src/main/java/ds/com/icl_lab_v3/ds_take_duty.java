...
public class ds_take_duty extends Fragment {
...
//        }
        System.out.println(list_of_ds.size());
        System.out.println(list_of_ds_name.size());
        System.out.println(list_of_duty.size());



        c=db.rawQuery("SELECT value FROM setting WHERE setting_name='lab_state'",null);
        String currlabstate="";
        while (c.moveToNext()){
            currlabstate=c.getString(0);
        }



        rootView = inflater.inflate(R.layout.ds_duty, container, false);
...
//        sp1.setAdapter(dslist);
        day_list.add("N/A");
        for (int i=0; i<list_of_duty.size(); i++){
            if (list_of_duty.get(i).equals(currlabstate)){
...
//        getMenuInflater().inflate(R.menu.menu_ds_substitute, menu);
//        return true;
//    }
    ...
    public void click_duty (View v){
        ...
        if (input_id.length()<8){
            ...
            if (list_of_ds.indexOf(input_id) == -1) {
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
                builder.setMessage("You are currently in duty, please go to 'Leave' tab to leave duty.");
                builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                    }
                });
...
//                refresh();
            ...
            else if (sp2.getSelectedItemPosition() == 0) {
                if (list_of_duty.get(list_of_ds.indexOf(input_id)).equals(create_session.lab_state)) {
                        ...
                        Date date = new Date();
                        textView.setText("");
                        db.execSQL("INSERT INTO duty_list VALUES(" + "'" + input_id + "'" + "," + "'" + "0" + "'"+ ");");
                        db.execSQL("INSERT INTO duty_log VALUES(" + "'" + input_id + "'" + ","+ "'" + "0" + "'" + "," + "'" + dateFormat.format(date) + "'" + "," + "'" + " " + "'" + ");");
...
//                refresh();
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
                    builder.setMessage("You are not in today's duty roster. To substitute others please select a member to substitute above.");
                    builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
                        }
                    });
...
//                refresh();
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
                    builder.setMessage("You have successfully substituted " + subname + " for his/her duty!");
                    builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
                        }
                    });
                    ...
                    Date date = new Date();
                    textView.setText("");
                    db.execSQL("INSERT INTO duty_list VALUES(" + "'" + input_id + "'" + "," + "'" + list_of_ds.get(list_of_ds_name.indexOf(subname)) + "'" + ");");
                    db.execSQL("INSERT INTO duty_log VALUES(" + "'" + input_id + "'" + "," + "'" + list_of_ds.get(list_of_ds_name.indexOf(subname)) + "'" + "," + "'" + dateFormat.format(date) + "'" + "," + "'" + " " + "'" + ");");
